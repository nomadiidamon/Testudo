    ...
    public Room(Vector3 position, int width, int height, int length, GameObject roomFloorPrefab)
    {
        ...
        for (int i = 0; i < objects.Count; i++)
        {
            switch (objects[i].name)
            {
                ...
                case "PG_Wall_North":
                    northWall = objects[i];
                    northWall.transform.position = Top_Middle.position;
                    walls.Add(northWall);
                    wallsToLocation.Add(northWall.transform.position, northWall);
                    break;
                case "PG_Wall_South":
                    southWall = objects[i];
                    southWall.transform.position = Bottom_Middle.position;
                    walls.Add(southWall);
                    wallsToLocation.Add(southWall.transform.position, southWall);
                    break;
                case "PG_Wall_East":
                    eastWall = objects[i];
                    eastWall.transform.position = Right_Middle.position;
                    walls.Add(eastWall);
                    wallsToLocation.Add(eastWall.transform.position, eastWall);
                    break;
                case "PG_Wall_West":
                    westWall = objects[i];
                    westWall.transform.position = Left_Middle.position;
                    walls.Add(westWall);
                    wallsToLocation.Add(westWall.transform.position, westWall);
                    break;
                ...
                case "Top_RightCorner":
                    ...
                    wallsToLocation.Add(northEast.transform.position, northEast);
                    break;
                case "Bottom_RightCorner":
                    southEast = objects[i];
                    southEast.transform.position = Bottom_Middle.position + Right_Middle.position;
                    wallsToLocation.Add(southEast.transform.position, southEast);
                    break;
                case "Bottom_LeftCorner":
                    southWest = objects[i];
                    southWest.transform.position = Bottom_Middle.position + Left_Middle.position;
                    wallsToLocation.Add(southWest.transform.position, southWest);
                    break;
                case "Top_LeftCorner":
                    northWest = objects[i];
                    northWest.transform.position = Top_Middle.position + Left_Middle.position;
                    wallsToLocation.Add(northWest.transform.position, northWest);
                    break;


                case "PG_Floor":
                    roomFloor = objects[i];
                    roomFloor.transform.position = roomFloor.transform.position.Add(y: -1*(height / 4));
                    break;
